...
1. **Probe**: Using Scapy, `tcpscan` crafts a raw layer-3 packet containing a TCP segment with the `SYN` flag enabled (`flags="S"`) and dispatches it via standard network routes.
...
3. **Teardown**: To maintain stealth and avoid leaving dangling semi-open TCP connections on the target machine, `tcpscan` immediately handles state termination by transmitting a resetting payload (`RST`).
...
* **Cleartext TCP Fallback (`tcp_probe`)**:
...
* `-p`: The target ports to inspect. Supports a single port (`80`), a port range (`20-25`), or a comma-separated array (`22,80,443`). If omitted, the tool scans a default group of 13 common services.
